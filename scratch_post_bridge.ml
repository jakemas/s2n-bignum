(* Post-bridge work: derive LENGTH, ABBREV newlen, prove s29 FILTER=REJ_SAMPLE,
   then ghost-state ABBREVs (coeffs_ymm3, cmp_mask, table_entry), then cmp_mask bridge.
   Lines 690-803. *)
...
(* cmp_mask bridge — with GSYM BIT_SUBWORD_256 / GSYM VMOVMSKPS_BYTE_EQ fix *)
e (SUBGOAL_THEN
      `val(cmp_mask:int64) =
...
Printf.printf "=== post-bridge done (cmp_mask established) ===\n%!";;
