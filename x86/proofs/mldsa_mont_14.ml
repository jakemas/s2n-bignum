(* ========================================================================= *)
(* ML-DSA Montgomery multiplication + complete butterfly (15 instructions)   *)
(* ========================================================================= *)

needs "x86/proofs/base.ml";;
needs "x86/proofs/utils/mldsa.ml";;

let mldsa_mont_15_mc = define_assert_from_elf "mldsa_mont_15_mc" "x86/mldsa/mldsa_mont_14.o"
[
  ...
  0xc5; 0x95; 0xfa; 0xc9   (* VPSUBD (%_% ymm1) (%_% ymm13) (%_% ymm1) *)
];;

let MLDSA_MONT_15_EXEC = X86_MK_EXEC_RULE mldsa_mont_15_mc;;

(* ------------------------------------------------------------------------- *)
(* Proof for complete 15-instruction butterfly sequence                      *)
(* ------------------------------------------------------------------------- *)

let MLDSA_MONT_15_CORRECT = prove
 (`forall pc a b c d.
        ensures x86
             (\s. bytes_loaded s (word pc) mldsa_mont_15_mc /\
                  read RIP s = word pc /\
                  read YMM0 s = d /\
                  read YMM1 s = a /\
                  read YMM2 s = b /\
                  read YMM8 s = c)
             (\s. read RIP s = word (pc + LENGTH mldsa_mont_15_mc) /\
                  ...
                  read YMM1 s = vpsubd (vpblendd (word 170) 
                                                 (vmovshdup (vpmuldq d (vpmuldq a c)))
                                                 (vpmuldq d (vpmuldq a (vmovshdup c))))
                                       (vpaddd (vpblendd (word 170) 
                                                         (vmovshdup (vpmuldq b c))
                                                         (vpmuldq b (vmovshdup c))) a))
             (MAYCHANGE [RIP] ,, MAYCHANGE [ZMM13; ZMM12; ZMM14; ZMM8; ZMM1] ,, MAYCHANGE SOME_FLAGS)`,
  REWRITE_TAC [SOME_FLAGS; fst MLDSA_MONT_15_EXEC] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_INIT_TAC "s0" THEN
  
  MAP_EVERY (fun n -> X86_STEPS_TAC MLDSA_MONT_15_EXEC [n] THEN
                      X86_SIMD_SIMPLIFY_TAC [vpmuldq; vmovshdup; vpblendd; vpsubd; vpaddd])
            (1--15) THEN
            
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  REWRITE_TAC[MAYCHANGE] THEN ASM_REWRITE_TAC[] THEN
  REWRITE_TAC[GSYM vpmuldq; GSYM vmovshdup; GSYM vpblendd; GSYM vpsubd; GSYM vpaddd] THEN
  REPEAT CONJ_TAC THEN TRY REFL_TAC);;

(* proof notes/old version *)
...
let MLDSA_MONT_15_CORRECT = prove
 (`forall pc a b c d.
  ...
  (* Instructions 1-9: Montgomery multiplication *)
  MAP_EVERY (fun n -> X86_STEPS_TAC MLDSA_MONT_15_EXEC [n] THEN
                      X86_SIMD_SIMPLIFY_TAC [vpmuldq; vmovshdup; vpblendd])
            (1--9) THEN
  
  (* Instruction 10: vpsubd *)
  X86_STEPS_TAC MLDSA_MONT_15_EXEC [10] THEN
  X86_SIMD_SIMPLIFY_TAC [vpmuldq; vmovshdup; vpblendd; vpsubd] THEN
  
  (* Instruction 11: vpaddd *)
  X86_STEPS_TAC MLDSA_MONT_15_EXEC [11] THEN
  X86_SIMD_SIMPLIFY_TAC [vpmuldq; vmovshdup; vpblendd; vpsubd; vpaddd] THEN
  
  (* Instruction 12: vmovshdup ymm13, ymm13 *)
  X86_STEPS_TAC MLDSA_MONT_15_EXEC [12] THEN
  X86_SIMD_SIMPLIFY_TAC [vpmuldq; vmovshdup; vpblendd; vpsubd; vpaddd] THEN
  
  (* Instruction 13: vpblendd ymm13, ymm13, ymm14, 0xAA *)
  X86_STEPS_TAC MLDSA_MONT_15_EXEC [13] THEN
  X86_SIMD_SIMPLIFY_TAC [vpmuldq; vmovshdup; vpblendd; vpsubd; vpaddd] THEN
  
  (* Instruction 14: vpaddd ymm8, ymm13, ymm12 *)
  X86_STEPS_TAC MLDSA_MONT_15_EXEC [14] THEN
  X86_SIMD_SIMPLIFY_TAC [vpmuldq; vmovshdup; vpblendd; vpsubd; vpaddd] THEN
  
  (* Instruction 15: vpsubd ymm1, ymm13, ymm1 *)
  X86_STEPS_TAC MLDSA_MONT_15_EXEC [15] THEN
  X86_SIMD_SIMPLIFY_TAC [vpmuldq; vmovshdup; vpblendd; vpsubd; vpaddd] THEN
...
(* ------------------------------------------------------------------------- *)
(* Analysis of the complete 15-instruction butterfly sequence               *)
(* ------------------------------------------------------------------------- *)
...
(* Instructions 1-9: Montgomery multiplication
   ...
   Instructions 14-15: Complete butterfly transformation
   - YMM8 = YMM13 + YMM12  (vpaddd ymm8, ymm13, ymm12)  -- final high output
   - YMM1 = YMM1 - YMM13   (vpsubd ymm1, ymm13, ymm1)   -- final low output
   
   This implements the complete NTT butterfly:
   (low, high) → (low + high, low - high)
   
   Where:
   - low = first_blend + a
   - high = second_blend + (a - first_blend)
   - final_low = (first_blend + a) - second_blend
   - final_high = second_blend + (a - first_blend)
*)
