(* ========================================================================= *)
(* ML-DSA Montgomery multiplication - 2 instruction sequence                 *)
...
(**** print_literal_from_elf "x86/mldsa/mldsa_mont_1.o";;
 ****)

let mldsa_mont_2instr_mc = define_assert_from_elf "mldsa_mont_2instr_mc" "x86/mldsa/mldsa_mont_1.o"
[
...
let MLDSA_MONT_2INSTR_EXEC = X86_MK_EXEC_RULE mldsa_mont_2instr_mc;;

(* ------------------------------------------------------------------------- *)
(* Proof.                                                                    *)
(* ------------------------------------------------------------------------- *)

let MLDSA_MONT_2INSTR_CORRECT = prove
 (`forall pc a b.
        ensures x86
             (\s. bytes_loaded s (word pc) mldsa_mont_2instr_mc /\
                  read RIP s = word pc /\
                  read YMM1 s = a /\
                  read YMM8 s = b)
             (\s. read RIP s = word (pc + LENGTH mldsa_mont_2instr_mc) /\
                  read YMM13 s = 
                  ...
                  ((word_join:(64)word->(64)word->(128)word)
                   ((word_join:(32)word->(32)word->(64)word)
                    (word_subword b (224,32):(32)word) (word_subword b (224,32):(32)word))
                  ((word_join:(32)word->(32)word->(64)word)
                    (word_subword b (160,32):(32)word) (word_subword b (160,32):(32)word)))
                  ((word_join:(64)word->(64)word->(128)word)
                   ((word_join:(32)word->(32)word->(64)word)
                    (word_subword b (96,32):(32)word) (word_subword b (96,32):(32)word))
                  ((word_join:(32)word->(32)word->(64)word)
                    (word_subword b (32,32):(32)word) (word_subword b (32,32):(32)word))))
             (MAYCHANGE [RIP] ,, MAYCHANGE [ZMM13; ZMM12] ,, MAYCHANGE SOME_FLAGS)`,
  REWRITE_TAC [SOME_FLAGS; fst MLDSA_MONT_2INSTR_EXEC] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_INIT_TAC "s0" THEN
  X86_STEPS_TAC MLDSA_MONT_2INSTR_EXEC (1--2) THEN
