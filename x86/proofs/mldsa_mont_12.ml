(* ========================================================================= *)
(* ML-DSA Montgomery multiplication + butterfly steps (13 instructions)     *)
(* ========================================================================= *)

needs "x86/proofs/base.ml";;
needs "x86/proofs/utils/mldsa.ml";;

let mldsa_mont_13_mc = define_assert_from_elf "mldsa_mont_13_mc" "x86/mldsa/mldsa_mont_12.o"
[
...
let MLDSA_MONT_13_EXEC = X86_MK_EXEC_RULE mldsa_mont_13_mc;;

(* ------------------------------------------------------------------------- *)
(* Proof for 13-instruction sequence                                         *)
(* ------------------------------------------------------------------------- *)

let MLDSA_MONT_13_CORRECT = prove
 (`forall pc a b c d.
        ensures x86
             (\s. bytes_loaded s (word pc) mldsa_mont_13_mc /\
                  read RIP s = word pc /\
                  read YMM0 s = d /\
                  read YMM1 s = a /\
                  read YMM2 s = b /\
                  read YMM8 s = c)
             (\s. read RIP s = word (pc + LENGTH mldsa_mont_13_mc) /\
                  ...
                  read YMM13 s = vpblendd (word 170) 
  ...
  REWRITE_TAC [SOME_FLAGS; fst MLDSA_MONT_13_EXEC] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_INIT_TAC "s0" THEN
  
  MAP_EVERY (fun n -> X86_STEPS_TAC MLDSA_MONT_13_EXEC [n] THEN
                      X86_SIMD_SIMPLIFY_TAC [vpmuldq; vmovshdup; vpblendd])
            (1--9) THEN
  
  X86_STEPS_TAC MLDSA_MONT_13_EXEC [10] THEN
  X86_SIMD_SIMPLIFY_TAC [vpmuldq; vmovshdup; vpblendd; vpsubd] THEN
  
  X86_STEPS_TAC MLDSA_MONT_13_EXEC [11] THEN
  X86_SIMD_SIMPLIFY_TAC [vpmuldq; vmovshdup; vpblendd; vpsubd; vpaddd] THEN
  
  X86_STEPS_TAC MLDSA_MONT_13_EXEC [12] THEN
  X86_SIMD_SIMPLIFY_TAC [vpmuldq; vmovshdup; vpblendd; vpsubd; vpaddd] THEN

  X86_STEPS_TAC MLDSA_MONT_13_EXEC [13] THEN
  X86_SIMD_SIMPLIFY_TAC [vpmuldq; vmovshdup; vpblendd; vpsubd; vpaddd] THEN
...
(* ------------------------------------------------------------------------- *)
(* Analysis of the 13-instruction sequence                                   *)
(* ------------------------------------------------------------------------- *)
...
(* Instructions 1-9: Montgomery multiplication (as proven before)
   ...
   This creates the second blended result that will be used for the final
   butterfly operations in instructions 14-15.
*)
