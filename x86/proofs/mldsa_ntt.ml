...
(* ------------------------------------------------------------------------- *)
(* Optimized x86_64 zetas from                                               *)
(* mldsa-native/mldsa/native/x86_64/src/x86_64_zetas.i                       *)
(* Section 1: "twiddles * q^-1" - Montgomery form optimizations              *)
(* ------------------------------------------------------------------------- *)

let mldsa_zetas_optimized_twiddles_qinv = define
 `mldsa_zetas_optimized_twiddles_qinv:int list =
...
(* ------------------------------------------------------------------------- *)
(* Section 2: "twiddles" - Direct twiddle factors (reordered for AVX2)       *)
(* ------------------------------------------------------------------------- *)
...
(* ------------------------------------------------------------------------- *)
(* ML-DSA pure NTT forms (analogous to ML-KEM) will add to common/           *)
(* ------------------------------------------------------------------------- *)
...
let mldsa_bitreverse_pairs = define
 `mldsa_bitreverse_pairs i = 2 * bitreverse8 (i DIV 2) + i MOD 2`;;

let mldsa_reorder = define
 `mldsa_reorder p (a:num->int) = \i. a(p i)`;;

(* ------------------------------------------------------------------------- *)
(* Conversion of each element of an array to Montgomery form with B = 2^32.  *)
(* ------------------------------------------------------------------------- *)
...
let MLDSA_FORWARD_NTT = prove
 (`mldsa_forward_ntt = mldsa_reorder mldsa_bitreverse_pairs o pure_forward_ntt_mldsa`,
...
let MLDSA_INVERSE_NTT = prove
 (`mldsa_inverse_ntt = tomont_8380417 o pure_inverse_ntt_mldsa o mldsa_reorder mldsa_bitreverse_pairs`,
  REWRITE_TAC[FUN_EQ_THM; o_DEF; mldsa_bitreverse_pairs; mldsa_reorder] THEN
  REWRITE_TAC[mldsa_inverse_ntt; pure_inverse_ntt_mldsa; tomont_8380417] THEN
  REWRITE_TAC[ARITH_RULE `(2 * x + i MOD 2) DIV 2 = x`] THEN
 ...
 g(`!a zetas pc.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST mldsa_ntt_tmc) /\
                  ...
                  read YMM0 s = word 0x007fe001007fe001007fe001007fe001007fe001007fe001007fe001007fe001)
  ...
  e(REWRITE_TAC[NONOVERLAPPING_CLAUSES; C_ARGUMENTS; fst MLDSA_NTT_TMC_EXEC]);;

  e(REPEAT STRIP_TAC);;

  (*** Manually expand the cases in the hypotheses ***)
  e(CONV_TAC(RATOR_CONV(LAND_CONV(ONCE_DEPTH_CONV EXPAND_CASES_CONV))));;
  ...
  (*** Simulate the execution with 50 steps ***)
  e(X86_STEPS_TAC MLDSA_NTT_TMC_EXEC (1--50));;
  (* hangs at first simulation :( *)
