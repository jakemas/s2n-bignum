(* ========================================================================= *)
(* ML-DSA Montgomery multiplication + butterfly steps (12 instructions)      *)
(* ========================================================================= *)

needs "x86/proofs/base.ml";;
needs "x86/proofs/utils/mldsa.ml";;

let mldsa_mont_12_mc = define_assert_from_elf "mldsa_mont_12_mc" "x86/mldsa/mldsa_mont_11.o"
[
...
let MLDSA_MONT_12_EXEC = X86_MK_EXEC_RULE mldsa_mont_12_mc;;

(* ------------------------------------------------------------------------- *)
(* Proof for 12-instruction sequence                                         *)
(* ------------------------------------------------------------------------- *)

let MLDSA_MONT_12_CORRECT = prove
 (`forall pc a b c d.
        ensures x86
             (\s. bytes_loaded s (word pc) mldsa_mont_12_mc /\
                  read RIP s = word pc /\
                  read YMM0 s = d /\
                  read YMM1 s = a /\
                  read YMM2 s = b /\
                  read YMM8 s = c)
             (\s. read RIP s = word (pc + LENGTH mldsa_mont_12_mc) /\
                  ...
                  read YMM13 s = vmovshdup (vpmuldq d (vpmuldq a c)))
             (MAYCHANGE [RIP] ,, MAYCHANGE [ZMM13; ZMM12; ZMM14; ZMM8; ZMM1] ,, MAYCHANGE SOME_FLAGS)`,
  REWRITE_TAC [SOME_FLAGS; fst MLDSA_MONT_12_EXEC] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_INIT_TAC "s0" THEN
  
  MAP_EVERY (fun n -> X86_STEPS_TAC MLDSA_MONT_12_EXEC [n] THEN
                      X86_SIMD_SIMPLIFY_TAC [vpmuldq; vmovshdup; vpblendd])
            (1--9) THEN
  
  X86_STEPS_TAC MLDSA_MONT_12_EXEC [10] THEN
  X86_SIMD_SIMPLIFY_TAC [vpmuldq; vmovshdup; vpblendd; vpsubd] THEN
  
  X86_STEPS_TAC MLDSA_MONT_12_EXEC [11] THEN
  X86_SIMD_SIMPLIFY_TAC [vpmuldq; vmovshdup; vpblendd; vpsubd; vpaddd] THEN
  
  X86_STEPS_TAC MLDSA_MONT_12_EXEC [12] THEN
  X86_SIMD_SIMPLIFY_TAC [vpmuldq; vmovshdup; vpblendd; vpsubd; vpaddd] THEN
...
(* ------------------------------------------------------------------------- *)
(* Analysis of the 12-instruction sequence                                   *)
(* ------------------------------------------------------------------------- *)
...
(* Instructions 1-9: Montgomery multiplication (as proven before)
   ...
   Instruction 12: Prepare for second blend
   - YMM13 = shuffle(d * (a * c))  (vmovshdup ymm13, ymm13)
   
   This prepares YMM13 for the second vpblendd operation in the next instruction,
   following the same pattern as instruction 2 prepared YMM8 for the first blend.
*)
