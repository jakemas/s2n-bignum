(* ========================================================================= *)
(* ML-DSA Montgomery multiplication - 6 instruction sequence                 *)
(* Using the successful explicit type annotation pattern                     *)
(* ========================================================================= *)

needs "x86/proofs/base.ml";;

(**** print_literal_from_elf "x86/mldsa/mldsa_mont_5.o";;
 ****)

let mldsa_mont_6instr_mc = define_assert_from_elf "mldsa_mont_6instr_mc" "x86/mldsa/mldsa_mont_5.o"
[
...
let MLDSA_MONT_6INSTR_EXEC = X86_MK_EXEC_RULE mldsa_mont_6instr_mc;;

(* ------------------------------------------------------------------------- *)
(* Basic execution proof                                                     *)
(* ------------------------------------------------------------------------- *)

let MLDSA_MONT_6INSTR_CORRECT = prove
 (`forall pc a b c d.
        ensures x86
             (\s. bytes_loaded s (word pc) mldsa_mont_6instr_mc /\
                  read RIP s = word pc /\
                  read YMM0 s = d /\
                  read YMM1 s = a /\
                  read YMM2 s = b /\
                  read YMM8 s = c)
             (\s. read RIP s = word (pc + LENGTH mldsa_mont_6instr_mc))
             (MAYCHANGE [RIP] ,, MAYCHANGE [ZMM13; ZMM12; ZMM14; ZMM8] ,, MAYCHANGE SOME_FLAGS)`,
  REWRITE_TAC [SOME_FLAGS; fst MLDSA_MONT_6INSTR_EXEC] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_INIT_TAC "s0" THEN
  X86_STEPS_TAC MLDSA_MONT_6INSTR_EXEC (1--6) THEN
...
let MLDSA_MONT_6INSTR_FUNCTIONAL = prove
 (`forall pc a b c d.
        ensures x86
             (\s. bytes_loaded s (word pc) mldsa_mont_6instr_mc /\
                  read RIP s = word pc /\
...
(*                                                                           *)
(* This extends the 5-instruction sequence with an additional multiplication *)
(* of YMM0 and the computed YMM13, storing the result back in YMM13.         *)
(* Final result: YMM13 = YMM0 * (YMM1 * YMM8)                                *)
(* ------------------------------------------------------------------------- *)
